...
class JenkinsIntegration:
    ...
    def get_build_console_log(self, job_name: str, build_number: int) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"Failed to fetch console log for {job_name}#{build_number}: {e}"
            )
            return None
    ...
    def get_all_jobs(self) -> List[str]:
        ...
        try:
            ...
            logger.error(
                f"Jenkins HTTP error: {e.response.status_code} - Check credentials"
            )
            return []
        except Exception as e:
    ...
    def poll_all_jobs(self, limit_per_job: int = 5, db=None) -> List[Dict]:
        ...
        for job_name in jobs:
            try:
                ...
                if failed_builds:
                    logger.info(
                        f"Found {len(failed_builds)} failed builds in {job_name}"
                    )
            except Exception as e:
