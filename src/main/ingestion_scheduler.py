        ...
        try:
            ...
            for build in failed_builds:
                try:
                    ...
                    if self.db.is_build_analyzed(build['job_name'], build['build_number']):
                        logger.debug(f"Skipping already analyzed: {build['job_name']}#{build['build_number']}")
                        continue
                    ...
                    analyzed_count += 1
                    logger.info(f"Analyzed and saved: {pipeline_name} - {result['category']}")
                except Exception as e:
                    logger.error(f"Error analyzing build {build.get('job_name', 'unknown')}: {e}")
                    continue
